...
export default function SpeciesScreen() {
  ...
  const handleOrganismPress = (org: OrganismMinimal) => {
    router.push(`/species/${org.id}`);
  };

  const handleAddSpecimenPress = (org: OrganismMinimal) => {
    router.push(`/specimen/create?organisme=${org.id}`);
  };

  const handleToggleFavori = async (org: OrganismMinimal) => {
        ...
        renderItem={({ item }) => (
          <View style={styles.card}>
            <TouchableOpacity
              ...
              onPress={() => handleOrganismPress(item)}
              activeOpacity={0.7}
            >
            ...
            <TouchableOpacity
              ...
              onPress={() => handleAddSpecimenPress(item)}
              hitSlop={{ top: 12, bottom: 12, left: 12, right: 12 }}
            >
